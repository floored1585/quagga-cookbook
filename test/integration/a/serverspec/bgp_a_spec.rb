...
describe file("#{cfg_dir}/bgpd.conf") do
  ...
  its(:content) { should contain("neighbor swp1 soft-reconfiguration inbound").after(/neighbor swp1 (peer-group$|remote-as|interface)/) }
  its(:content) { should contain("neighbor swp1 next-hop-self").after(/neighbor swp1 (peer-group$|remote-as|interface)/) }
  ...
  its(:content) { should contain("neighbor 2001:db8::abcd peer-group hosts_v6").after('hosts_v6 peer-group') }
  its(:content) { should contain("neighbor 2001:db8::abcd activate").after(/neighbor 2001:db8::abcd (peer-group\s+\S+|remote-as|interface)/) }
  its(:content) { should contain("neighbor 2001:db8::abcd description IPV6 DOC").after(/neighbor 2001:db8::abcd (peer-group\s+\S+|remote-as|interface)/) }
