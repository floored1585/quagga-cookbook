...
describe file("#{cfg_dir}/zebra.conf") do
  ...
  its(:content) { should contain('route-map MAP_A1 10 permit') }
  its(:content) { should contain('set aggregator as 1234 12.34.56.78') }
  its(:content) { should contain('set local-preference 100') }
  its(:content) { should contain('route-map MAP_A1 20 deny') }
  its(:content) { should contain('route-map MAP_A2 5 deny') }
